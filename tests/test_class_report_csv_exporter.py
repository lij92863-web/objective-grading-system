...
class ClassReportCsvExporterTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_field_order(self):
        from app.infrastructure.exporters.class_report_csv_exporter import CLASS_REPORT_FIELDNAMES
        t = tempfile.mkdtemp(prefix="t_", dir=PROJECT_ROOT/"data")
        try:
            ld = Path(t)/"l"; self._run(ld)
            with (ld/"class_report.csv").open("r", encoding="utf-8-sig", newline="") as h:
                self.assertEqual(list(CLASS_REPORT_FIELDNAMES), csv.DictReader(h).fieldnames)
    ...
    def test_round_trip(self):
        ...
        t = tempfile.mkdtemp(prefix="t_", dir=PROJECT_ROOT/"data")
        try:
            ld = Path(t)/"l"; self._run(ld); lr = self._read(ld/"class_report.csv")
            nd = Path(t)/"n"; nd.mkdir()
            r = ClassReportCsvExporter().export(ExportRequest(output_dir=nd), lr)
            self.assertEqual(r.status, "ok")
            nr = self._read(nd/"class_report.csv")
    ...
    def test_encoding(self):
        ...
        try:
            ld = Path(t)/"l"; self._run(ld); lr = self._read(ld/"class_report.csv")
            nd = Path(t)/"n"; nd.mkdir(); ClassReportCsvExporter().export(ExportRequest(output_dir=nd), lr)
            raw = (nd/"class_report.csv").read_bytes()
    ...
    def test_empty_rows(self):
        ...
        t = tempfile.mkdtemp(prefix="t_", dir=PROJECT_ROOT/"data")
        try:
            r = ClassReportCsvExporter().export(ExportRequest(output_dir=Path(t)), [])
            self.assertEqual(r.status, "ok"); self.assertIn("class_report_rows_empty", r.warnings)
        finally: shutil.rmtree(t, ignore_errors=True)

    def test_no_legacy(self):
        f = PROJECT_ROOT/"app/infrastructure/exporters/class_report_csv_exporter.py"
    ...
    def test_old_still_passes(self):
        t = tempfile.mkdtemp(prefix="t_", dir=PROJECT_ROOT/"data")
        try: self.assertTrue(self._run(Path(t))["ok"]); self.assertTrue((Path(t)/"class_report.csv").exists())
        finally: shutil.rmtree(t, ignore_errors=True)
